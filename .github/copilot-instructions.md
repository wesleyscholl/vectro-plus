...
| `VECTRO+QSTREAM1` | `VECTRO+QSTREAM1\n` | `u32 tables` + `u32 dim` + `u32 blob_len` + bincode quant tables + repeated encoded records | Production |
| `VECTRO+PQSTREAM1` | `VECTRO+PQSTREAM1\n` | Codebook blob + repeated `u32 len` + `bincode((id, code: Vec<u8>))` | Planned v1.2.0 |
...
**Never break format backward compatibility.** New format = new magic header. `EmbeddingDataset::load()` must identify all formats correctly — add a format-detection test for every new format before merging.
...
- `vectro_lib`: `rayon` for data-parallel work (quantization, search).
- `vectro_cli` `compress_stream()`: `crossbeam-channel` reader → worker pool → writer pipeline.
- `vectro_cli` `server.rs`: `tokio` async I/O; `AppState` guarded by `Arc<RwLock<...>>`.
- **Never mix rayon and tokio on the same thread pool.** Rayon work from async context → `tokio::task::spawn_blocking()`.
...
| `/api/search` | POST | JSON `{query: [f32], k: usize}`; returns `[{id, score}]` sorted descending |
| `/api/upload` | POST | Accepts JSONL or bincode stream; loads into AppState |
| `/api/load` | GET | Loads dataset from server-local file path |

**Validate all inputs at the API boundary.** Wrong dimension → HTTP 400 with clear message. Never panic or silently truncate. Timeouts are mandatory on all endpoints.

---

## 💹 Performance Contracts

Hard baselines on M3 16GB, release build. Regressions are a hard stop — profile with Criterion before merging.

| Operation | Dataset | Contract |
|---|---|---|
| STREAM1 compress | 10k × 768d | p95 < 500 ms |
| QSTREAM1 compress | 10k × 768d | p95 < 800 ms |
| Top-10 cosine search (brute force) | 10k × 768d | p95 < 500 μs |
| Top-100 cosine search (brute force) | 10k × 768d | p95 < 2 ms |
| REST `/api/search` roundtrip | in-memory | p95 < 5 ms |
| `cargo test` | all crates | < 30 s |

All benchmarks use Criterion (`cargo bench`). All benchmarks live in `vectro_lib/benches/quant_bench.rs`.

---

## 🔬 Recall & Compression Validation Gates

**For any new quantization format (PQ, NF4, RQ):**
1. Compression ratio ≥ target (PQ: ≥ 16×, RQ: ≥ 32×).
2. `recall@10` ≥ 0.90 vs brute-force on 10k-vector toy dataset.
3. Encode/decode roundtrip test with asserted max absolute error snapshot.
4. Format detection test for `EmbeddingDataset::load()`.

**For HNSW or any ANN index:**
1. `recall@10` ≥ 0.95 vs brute-force on 100k-vector dataset.
2. QPS ≥ 5000 on M3 at that recall level.
3. Save/load roundtrip produces identical results.

**Recall drop >2pp on any validated config = hard stop.**

---

## 🐍 Python Bindings (PyO3 Rules)

- `vectro_py` is a thin wrapper — no algorithm logic.
- `PyReadonlyArray1<f32>` borrows Python's buffer — convert to `Vec<f32>` before any thread boundary.
- All `#[pymethods]` return `PyResult<T>` — never panic. Use `.map_err(|e| PyRuntimeError::new_err(e.to_string()))`.
- Type stubs (`python/vectro_plus/__init__.pyi`) updated in same commit as any signature change.
- Build toolchain: Maturin (`maturin develop` / `maturin build --release --strip`). Do not extend `setup.py`.
...
|---|---|---|---|
| Product Quantization | v1.2.0 | `vectro_lib/src/pq.rs`, `VECTRO+PQSTREAM1`, `--format pq` | recall@10 ≥ 0.90, ≥ 16× compression |
| HNSW ANN Index | v1.3.0 | `vectro_lib/src/hnsw.rs`, `vectro index` CLI, `/api/index/*` | recall@10 ≥ 0.95, QPS ≥ 5000 |
| Real-World Benchmarks | v1.4.0 | `BENCHMARKS.md`, SIFT1M + GloVe-100d, `--ground-truth` eval | recall@10 ≥ 0.90 on SIFT1M, CI reproducible |
| PyPI Distribution | v1.5.0 | Maturin wheels, `pip install vectro-plus`, type stubs, GitHub Actions | wheel installs, CI Python tests pass |
| Research Port | v2.0.0 | NF4, RQ, AutoQuantize from `~/vectro/` (Mojo v3.6.0); `--format auto` | δ recall@10 < 5% vs vectro on GloVe-100d |
...
```bash
cargo test                          # full test suite (must pass before commit)
cargo clippy -- -D warnings         # zero warnings policy
cargo bench                         # Criterion benchmarks
cargo build --release               # release binary
maturin develop                     # Python dev install
maturin build --release --strip     # Python wheel
...
1. `cargo test` — zero failures across all crates.
2. `cargo clippy -- -D warnings` — zero warnings.
3. Performance contracts within spec or written exception filed.
4. Recall/compression gate passed for any format or search change.
5. `QSTREAM.md` updated for any new binary format.
6. `CHANGELOG.md` entry under the correct version heading.
7. `README.md` updated if public API, CLI flags, or Python API changed.
8. Type stubs (`__init__.pyi`) updated in same commit as any Python binding change.
